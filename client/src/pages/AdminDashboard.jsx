...
const AdminDashboard = () => {
  ...
  const fetchData = useCallback(async () => {
    try {
      setIsLoading(true);
      const [allUsers, allTheses, pendingData, approvedData] = await Promise.all([
              ...
              <div className="space-y-4">
                {filteredStudents.length > 0 ? (
                  filteredStudents.map((student) => {
                    // Find the reviewer specific to this student
                    const reviewer = approvedReviewers.find(
                      (reviewer) => reviewer._id === student.reviewer
                    );

                    const thesis = theses?.find(
                      (thes) => thes.student === student._id
                    );
